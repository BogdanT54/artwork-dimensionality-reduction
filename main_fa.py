...
def main():
    functii.goleste_data_out(tokens=["_FA.", "Bartlett_KMO_FA", "Comunalitati_FA",
                                       "Incarcare_FA", "Scoruri_FA", "Varianta_FA",
                                       "Loadings_FA", "Scatter_FA"])
    df = pd.read_csv(functii.DATA_IN / "features_cnn.csv")
    metadata = df[META_COLS].copy()
    x = df.drop(columns=META_COLS).values.astype(np.float32)

    # FA cu factor_analyzer e costisitor pe 4096 dim; reducem features la top-variance ~500
    var_per_feat = x.var(axis=0)
    top_feat = np.argsort(-var_per_feat)[:500]
    x_top = x[:, top_feat]
    print(f"[info] FA pe top-500 features cu cea mai mare varianță; X = {x_top.shape}")
